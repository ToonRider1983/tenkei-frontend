...
const Button = ({ label, subLabel, to }) => (
    <Link to={to} className="bg-[#6A9C89] hover:bg-[#45695C] text-white font-medium h-24 w-auto mr-2.5 ml-2.5 py-2 px-4 rounded-lg shadow-md flex justify-center items-center text-center">
        <div>
            {label} <br /> {subLabel && <span>{subLabel}</span>}
        </div>
    </Link>
);
...
export default function DashboardPage() {
    ...
    return (
        <div className="flex bg-[#E9EFEC]">
            <Sidebar />
            <div className="flex flex-col mr-2 ml-2 w-screen">
                <Navbar />
                <p className="font-bold text-3xl mt-5 ml-10">TENKEI Client Menu</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-5 p-6 flex-1">
